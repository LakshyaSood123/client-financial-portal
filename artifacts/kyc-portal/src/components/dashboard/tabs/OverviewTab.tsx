...
// ── Warm surface tokens ────────────────────────────────────────
const CREAM  = "#FAF8F4";   // ivory — KPIs, primary cards
const WARM   = "#F2EBE1";   // warm beige — charts
const BLUSH  = "#EBE1D5";   // deeper blush — support/bottom
...
export function OverviewTab() {
  return (
    ...
    >
      {/* KPI Cards — cream surface */}
      <div className="grid grid-cols-4 gap-4 mb-6">
        <KPICard title="KYB Status" value="Verified" percent={100} color="#22C55E"  delay={0.05} />
        <KPICard title="Ops Status" value="Active"   percent={90}  color="#F59E0B"  delay={0.1} />
        <KPICard title="Risk Tier"  value="Low"      percent={75}  color="#8b6ff4"  delay={0.15} />
        <KPICard title="Plan"       value="Growth"   percent={60}  color="#F97316"  delay={0.2} />
      </div>

      {/* Usage Chart (warm) + Webhook Donut (warm) */}
      <div className="grid grid-cols-12 gap-5 mb-5">
        <UsageChart />
        <WebhookDonut />
      </div>

      {/* Bottom summary cards — blush surface */}
