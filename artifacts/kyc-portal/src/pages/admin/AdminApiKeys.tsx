    ...
    <AdminLayout>
      <div style={{ padding: "28px 32px" }}>
        ...
        <div style={{
          ...
          background: "rgba(248,113,113,0.1)", borderRadius: 10, padding: "8px 14px",
          border: "1px solid rgba(248,113,113,0.2)", marginBottom: 20, width: "fit-content",
        }}>
        ...
        {/* UI shell preview */}
        <div style={{ ...cardShell, background: SURF_DEFAULT, padding: 24, opacity: 0.6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <Key style={{ width: 18, height: 18, color: MUTED }} />
            <span style={{ fontSize: 15, fontWeight: 700, color: TEXT }}>Platform Keys</span>
          </div>
          {["Integration-Webhook-Delivery", "Compliance-Export-Service", "Analytics-Aggregation"].map(name => (
            <div
              key={name}
              style={{
                background: SURF_SUPPORT, borderRadius: 10, padding: "12px 16px",
                marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between",
              }}
            >
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: TEXT, marginBottom: 3 }}>{name}</div>
                <code style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: MUTED }}>
                  sk_adm_••••••••••••••••
                </code>
              </div>
              <span style={{ fontSize: 11.5, color: MUTED, fontStyle: "italic" }}>backend pending</span>
            </div>
          ))}
        </div>
      </div>
