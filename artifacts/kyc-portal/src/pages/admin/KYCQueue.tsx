...
const QUEUE_ITEMS = [
  ...
  { id: "KYC-2843", name: "GlobalPay Inc", type: "Business", email: "compliance@globalpay.io", submitted: "4 hours ago", docs: 7, risk: "Medium", country: "UK", flags: ["Director Verification Needed"] },
  { id: "KYC-2840", name: "Maria Santos", type: "Individual", email: "m.santos@mail.com", submitted: "6 hours ago", docs: 2, risk: "Low", country: "Brazil", flags: [] },
  { id: "KYC-2838", name: "Vertex Capital", type: "Business", email: "kyc@vertexcap.com", submitted: "8 hours ago", docs: 9, risk: "High", country: "Cayman Islands", flags: ["High Risk Jurisdiction", "Complex Structure"] },
  { id: "KYC-2835", name: "Priya Sharma", type: "Individual", email: "priya.s@outlook.com", submitted: "10 hours ago", docs: 3, risk: "Low", country: "India", flags: [] },
  { id: "KYC-2831", name: "NexGen Logistics", type: "Business", email: "admin@nexgen.co", submitted: "12 hours ago", docs: 6, risk: "Medium", country: "UAE", flags: ["UBO Verification Pending"] },
  { id: "KYC-2828", name: "Thomas Baker", type: "Individual", email: "t.baker@gmail.com", submitted: "14 hours ago", docs: 3, risk: "Low", country: "USA", flags: [] },
];

const RISK_STYLES: Record<string, { color: string; bg: string; className: string }> = {
  Low: { color: "#22C55E", bg: "rgba(34,197,94,0.1)", className: "admin-pill admin-pill-positive" },
  Medium: { color: "#d97706", bg: "rgba(217,119,6,0.1)", className: "admin-pill admin-pill-warning" },
  High: { color: "#f54a4a", bg: "rgba(245,74,74,0.1)", className: "admin-pill admin-pill-negative" },
};
...
export default function KYCQueue() {
  ...
  const filtered = useMemo(() => {
    const base = filter === "All" ? QUEUE_ITEMS : QUEUE_ITEMS.filter((i) => i.risk === filter || i.type === filter);
    ...
    return base.filter((item) =>
      [item.id, item.name, item.email, item.country, item.risk, ...item.flags]
        .join(" ")
        .toLowerCase()
        ...
        <motion.div className="flex items-center gap-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          {["All", "Individual", "Business", "High", "Medium", "Low"].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-all admin-button ${filter === f ? "admin-button-primary" : ""}`}
              style={filter === f ? { background: DARK_1, color: "#ffffff" } : { background: SURF_SUPPORT, color: MUTED, border: "1px solid rgba(13,18,33,0.07)" }}
            >
              {f}
            </button>
          ))}
              ...
              <motion.div key={item.id} className="admin-task-card" style={{ ...cardShell, background: SURF_DEFAULT, borderRadius: 22, opacity: decision ? 0.55 : 1 }} initial={{ opacity: 0, y: 12 }} animate={{ opacity: decision ? 0.55 : 1, y: 0 }} transition={{ delay: 0.25 + i * 0.06 }}>
                <div className="flex items-center px-5 h-16 gap-4">
                  ...
                  <div className="hidden lg:flex items-center gap-6 shrink-0">
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-bold ${riskStyle.className}`} style={{ background: riskStyle.bg, color: riskStyle.color }}>
                      {item.risk} Risk
                    </div>
                  </div>

                  {decision ? (
                    <div className={decision === "approved" ? "admin-pill admin-pill-positive" : "admin-pill admin-pill-negative"}>
                      {decision === "approved" ? "✓ Approved" : "✕ Rejected"}
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <button onClick={() => setExpanded(isExpanded ? null : item.id)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors admin-icon-button" style={{ color: MUTED, background: "rgba(13,18,33,0.05)" }}>
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => reject(item.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all admin-button" style={{ background: "rgba(245,74,74,0.1)", color: "#f54a4a", border: "1px solid rgba(245,74,74,0.18)" }}>
                        <XCircle className="w-3.5 h-3.5" /> Reject
                      </button>
                      <button onClick={() => approve(item.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all admin-button" style={{ background: "rgba(22,163,74,0.1)", color: "#16a34a", border: "1px solid rgba(22,163,74,0.18)" }}>
                      ...
                      <div className="mx-5 mb-4 p-4 rounded-2xl grid grid-cols-3 gap-6" style={{ background: SURF_SUPPORT, border: "1px solid rgba(13,18,33,0.06)" }}>
                        <div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            {item.docs > 3 && <p className="text-xs" style={{ color: MUTED }}>+{item.docs - 3} more documents</p>}
                          </div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="space-y-1.5 text-sm">
                            <div className="flex justify-between"><span style={{ color: MUTED }}>Country</span><span style={{ color: TEXT }}>{item.country}</span></div>
                            <div className="flex justify-between"><span style={{ color: MUTED }}>Type</span><span style={{ color: TEXT }}>{item.type}</span></div>
                            <div className="flex justify-between"><span style={{ color: MUTED }}>Risk</span><span style={{ color: riskStyle.color }}>{item.risk}</span></div>
                          </div>
                        </div>
                      </div>
                    </motion.div>
