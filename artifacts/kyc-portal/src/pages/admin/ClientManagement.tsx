...
const KYB_STYLES: Record<string, { color: string; bg: string }> = {
  Verified: { color: "#a8ff3e", bg: "rgba(168,255,62,0.12)" },
  Pending: { color: "#ffb547", bg: "rgba(255,181,71,0.12)" },
  Rejected: { color: "#ff5a5a", bg: "rgba(255,90,90,0.12)" },
};
      ...
      <div className="p-8 space-y-6">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "#6b8a82" }}>
        ...
        >
          <div className="relative flex-1 max-w-sm">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "#6b8a82" }} />
            <input
              type="text"
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search by name, email, ID…"
              className="w-full h-10 pl-11 pr-4 text-sm text-foreground placeholder:text-[#3d5a52] outline-none rounded-2xl"
          ...
          <div className="flex items-center gap-2">
            {["All", "Verified", "Pending", "Rejected"].map(f => (
              <button
                ...
                style={kybFilter === f ? {
                  background: f === "All" ? "#ff5a5a" : KYB_STYLES[f]?.bg ?? "#ff5a5a",
                  color: f === "All" ? "#050c0e" : KYB_STYLES[f]?.color ?? "#ff5a5a",
                  border: "none",
                } : {
                  background: "rgba(255,255,255,0.04)",
                  color: "#6b8a82",
        ...
        >
          ...
          <div>
            {filtered.map((client, i) => (
              <motion.div
                ...
                whileHover={{ background: "rgba(255,255,255,0.03)", borderLeftColor: "#00d4aa" } as React.CSSProperties}
              >
                <span className="font-mono text-xs" style={{ color: "#3d5a52" }}>{client.id}</span>

                <div className="flex items-center gap-3 min-w-0">
                  <div
                    className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                    style={{ background: client.type === "Business" ? "rgba(155,127,244,0.2)" : "rgba(0,212,170,0.12)" }}
                  >
                    {client.type === "Business"
                      ? <Building2 className="w-4 h-4" style={{ color: "#9b7ff4" }} />
                      : <User className="w-4 h-4" style={{ color: "#00d4aa" }} />
                    }
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-foreground group-hover:text-[#00d4aa] transition-colors truncate">{client.name}</p>
                    <p className="text-xs truncate" style={{ color: "#6b8a82" }}>{client.email}</p>
                  </div>
                </div>

                <span className="text-xs" style={{ color: "#6b8a82" }}>{client.country}</span>
                ...
                <span className="text-xs" style={{ color: client.plan === "—" ? "#3d5a52" : "#f0f8f5" }}>{client.plan}</span>

                <span className="text-sm font-mono font-bold" style={{ color: client.revenue === "$0" ? "#3d5a52" : "#a8ff3e" }}>
                  {client.revenue}
                </span>
              </motion.div>
            ))}
          ...
          <div
            ...
            style={{ color: "#6b8a82", borderTop: "1px solid rgba(255,255,255,0.06)" }}
          >
            <span>Showing {filtered.length} of {CLIENTS.length} clients</span>
            <div className="flex items-center gap-2">
              <button className="px-3 py-1.5 rounded-lg transition-colors hover:bg-white/10">← Prev</button>
              <span className="px-3 py-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.06)", color: "#f0f8f5" }}>1</span>
              <button className="px-3 py-1.5 rounded-lg transition-colors hover:bg-white/10">Next →</button>
            </div>
          </div>
        </motion.div>
      </div>
